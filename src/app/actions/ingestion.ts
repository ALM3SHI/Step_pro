...
export type SaveCategory = 'grammar' | 'reading' | 'listening' | 'writing';
...
export async function saveHybridBatch(input: {
  ...
  if (!input.questions.length) return { ok: false, error: 'لا توجد أسئلة للحفظ' };
  if (input.questions.length > MAX_QUESTIONS_PER_CALL) {
    return {
      ok: false,
      error: `${input.questions.length} سؤال يتجاوز حد ${MAX_QUESTIONS_PER_CALL} لكل طلب — قسّمها.`,
    };
  }
  ...
  if (missingKey.length) {
    return { ok: false, error: `${missingKey.length} سؤال بلا إجابة صحيحة صالحة` };
  }
...
export async function saveListeningBatch(input: {
  ...
  if (missingKey.length) return { ok: false, error: `${missingKey.length} سؤال بلا إجابة صحيحة` };
  ...
  try {
    ...
    const { data: batch, error: batchErr } = await db
      ...
      .insert({
        ...
        source_metadata: { notes: input.sourceNotes ?? '', mode: 'hybrid', category: 'listening', audioKey: input.audioKey },
        status: 'processing',
        total_questions_parsed: input.questions.length,
